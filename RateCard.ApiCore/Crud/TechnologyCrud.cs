...
{
    public class TechnologyCrud : CrudFactory
    {
        private TechnologyMapper _Mapper_Technology;

        public TechnologyCrud()
        {
            _Mapper_Technology = new TechnologyMapper();
        }
        ...
        {
            var operation = _Mapper_Technology.GetRetrieveAllStatement();
            var instance = SqlDao.GetInstance();
            var listResult = instance.ExecuteQueryProcedure(operation);
            List<BaseEntity> list = _Mapper_Technology.BuildObjects(listResult);
